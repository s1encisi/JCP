...
# -*- coding: utf-8 -*-
"""
run_benchmark_ablation_no_time_power.py

【消融实验 1 — 分工况建模，去除时间特征与功率特征】

控制变量设计：
  原代码（基准）：分工况 + year/month/day/hour + 功率特征
  本脚本（消融）：分工况（结构不变）+ 去掉 year/month/day/hour + 去掉功率特征
...
严格控制变量措施：
  ...
  唯一改变的变量：输入特征去掉 year/month/day/hour 及 *_power 列。

输出目录：benchmarkoutputs/ablation_no_time_power/
"""
...
OUTPUT_DIR   = 'benchmarkoutputs/ablation_no_time_power'
BENEFIT_COLS = ('Test_R2',)
COST_COLS    = ('Test_RMSE', 'Test_MAPE')
WEIGHTS      = [1, 1, 1]

# ─────────────────────────────────────────────────────────────────────────────
# 消融特征集：动态从原始特征列表中剔除时间/功率列
# ─────────────────────────────────────────────────────────────────────────────
_REMOVE = {'year', 'month', 'day', 'hour',
           'three_stage_power', 'four_stage_power', 'total_power'}

ABLATION_FEATURES = {
    task_name: {
        mode: [f for f in orig_feats if f not in _REMOVE]
        for mode, orig_feats in ORIG_TASK_CONFIGS[task_name]['features'].items()
    }
...
def main():
    print("=" * 80)
    print("消融实验 1：分工况建模 + 去除 year/month/day/hour 及功率特征")
    ...
    for task_name in ['Cu', 'As', 'Voltage']:
        ...
        output_path = os.path.join(OUTPUT_DIR,
                                   f'ablation_no_time_power_{task_name}.xlsx')
