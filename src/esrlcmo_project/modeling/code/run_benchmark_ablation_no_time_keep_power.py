...
# -*- coding: utf-8 -*-
"""
run_benchmark_ablation_no_time_keep_power.py

【消融实验 3 — 分工况建模，仅去除时间特征，保留功率特征】

控制变量设计：
  原代码（基准）：分工况 + year/month/day/hour + 功率特征
  本脚本（消融）：分工况（结构不变）+ 去掉 year/month/day/hour + 保留功率特征
...
严格控制变量措施：
  ...
  唯一改变的变量：输入特征去掉 year/month/day/hour，功率列原样保留。

输出目录：benchmarkoutputs/ablation_no_time_keep_power/
"""
...
OUTPUT_DIR   = 'benchmarkoutputs/ablation_no_time_keep_power'
BENEFIT_COLS = ('Test_R2',)
COST_COLS    = ('Test_RMSE', 'Test_MAPE')
WEIGHTS      = [1, 1, 1]

# ─────────────────────────────────────────────────────────────────────────────
# 消融特征集：仅剔除时间列，功率列原样保留
# ─────────────────────────────────────────────────────────────────────────────
_REMOVE_TIME = {'year', 'month', 'day', 'hour'}

ABLATION_FEATURES = {
    task_name: {
        mode: [f for f in ORIG_TASK_CONFIGS[task_name]['features'][mode]
               if f not in _REMOVE_TIME]
...
def main():
    print("=" * 80)
    print("消融实验 3：分工况建模 + 仅去除 year/month/day/hour，保留功率特征")
    print("对比基准  ：原代码（分工况 + 含时间/功率特征）")
    print("唯一变量  ：输入特征去掉时间列（year/month/day/hour），功率列保留")
    print("=" * 80)
    ...
    for task_name in ['Cu', 'As', 'Voltage']:
        ...
        for mode in OPERATION_MODES:
            ...
            kept_power = [f for f in ablation_cols if 'power' in f]
        ...
        output_path = os.path.join(OUTPUT_DIR,
                                   f'ablation_no_time_keep_power_{task_name}.xlsx')
