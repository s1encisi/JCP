...
- 包含三个独立模型：
  1. three_stage模型：仅使用三段数据，预测三段电压V
     特征：增加 year month day hour
  2. four_stage模型：仅使用四段数据，预测四段电压V
     特征：增加 year month day hour
  3. serial模型：使用串联数据，预测三四段平均电压
     特征：增加 year month day hour
- 每个模型保持独立配置和训练流程
...
CONFIG_FILE = str(MODEL_OUTPUT_ROOT / 'voltage_config.json')
...
# three_stage 模型配置
# 特征：增加 year month day hour，预测三段电压V
THREE_STAGE_CONFIG = {
    ...
    'OUTPUT_DIR': str(MODEL_OUTPUT_ROOT / 'voltage_three_stage'),
...
# four_stage 模型配置（四段建模）
# 特征：增加 year month day hour，预测四段电压V
FOUR_STAGE_CONFIG = {
    'USE_OPTIMIZED_RANDOM_STATE': False,
    'MANUAL_RANDOM_STATE': 274,
    'USE_BAYESIAN_OPTIMIZATION': False,
    'OUTPUT_DIR': str(MODEL_OUTPUT_ROOT / 'voltage_four_stage'),
    ...
    'PREDICTION_TARGET': '四段电压V',
    'BEST_RANDOM_STATE': 400,
    'BEST_PARAMS': {
        'n_estimators': 100,
        'max_depth': 10,
        'min_samples_split': 2,
...
# serial 模型配置（串联建模）
# 特征：增加 year month day hour，预测三四段平均电压
SERIAL_CONFIG = {
    'USE_OPTIMIZED_RANDOM_STATE': False,
    'MANUAL_RANDOM_STATE': 166,
    'USE_BAYESIAN_OPTIMIZATION': False,
    'OUTPUT_DIR': str(MODEL_OUTPUT_ROOT / 'voltage_serial'),
    'SHEET_NAME': 'serial',
    'MODEL_TAG': 'serial',
    'FEATURE_COLS': [
        '电积前液Cu（g/L）',
    ...
    'PREDICTION_TARGET': '三四段平均电压V',
    'BEST_RANDOM_STATE': 101,
    'BEST_PARAMS': {
        'n_estimators': 100,
        'max_depth': 10,
        'min_samples_split': 2,
...
if __name__ == "__main__":
    ...
    print("三段预测三段电压V，四段预测四段电压V，串联预测三四段平均电压V")
    print("=" * 120)
    ...
    three_stage_train_metrics, three_stage_test_metrics, three_stage_best_rs = train_model(THREE_STAGE_CONFIG, unit='V')
    four_stage_train_metrics, four_stage_test_metrics, four_stage_best_rs = train_model(FOUR_STAGE_CONFIG, unit='V')
    serial_train_metrics, serial_test_metrics, serial_best_rs = train_model(SERIAL_CONFIG, unit='V')
    ...
    tag_map = {
        'three_stage': ('outputs/voltage_three_stage', 'prediction_results_three_stage_train.xlsx',
                        'prediction_results_three_stage_test.xlsx'),
        'four_stage':  ('outputs/voltage_four_stage',  'prediction_results_four_stage_train.xlsx',
                        'prediction_results_four_stage_test.xlsx'),
        'serial':      ('outputs/voltage_serial',       'prediction_results_serial_train.xlsx',
    ...
    try:
        ...
        for tag, (out_dir, train_file, test_file) in tag_map.items():
            ...
            col_map = {'three_stage': '三段电压V', 'four_stage': '四段电压V', 'serial': '三四段平均电压V'}
            col = col_map[tag]
    ...
    try:
        ...
        # 创建输出目录
        output_dir = 'outputs/voltage_metrics'
        ...
        # 输出到Excel文件
        excel_path = f'{output_dir}/voltage_metrics_summary.xlsx'
